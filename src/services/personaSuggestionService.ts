...
function buildPrompt(contact: any, personas: any[]): string {
...
function cleanAndParseJson(aiText: string): PersonaSuggestionResult | null {
  ...
  try {
    ...
    if (
      ...
      return parsed as PersonaSuggestionResult;
    }
  } catch (_) {
    return null;
  }
...
export async function suggestPersonaForContact(
  userId: number,
  contactId: string,
  provider: string,
  apiKey: string
): Promise<PersonaSuggestionResult | { error: string }> {
  // 1) Fetch contact
  ...
  const personasRes = await Database.query(
    'SELECT id, name, description FROM personas WHERE user_id = $1 ORDER BY created_at DESC',
    [userId]
  );
