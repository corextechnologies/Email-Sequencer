...
export class AuthService {
   ...
   */
  static async sendRegistrationOTP(email: string, password: string): Promise<void> {
    // Validate password requirements
    ...
    if (existingUser.rows.length > 0 && !existingUser.rows[0].email_verified) {
      ...
      await Database.query(
        `UPDATE users 
         SET verification_code = $1, 
             ...
             updated_at = CURRENT_TIMESTAMP
         WHERE id = $4`,
        [hashedCode, expiresAt, tempPasswordHash, existingUser.rows[0].id]
      );
      ...
      await Database.query(
        `INSERT INTO users (email, password_hash, temp_password_hash, verification_code, verification_expires_at, email_verified)
         VALUES ($1, $2, $3, $4, $5, false)
         RETURNING id`,
        [email, tempPasswordHash, tempPasswordHash, hashedCode, expiresAt]
      );
    ...
    try {
      await TransactionalEmailService.sendRegistrationOTPEmail(email, verificationCode);
   ...
   */
  static async verifyRegistrationOTP(email: string, code: string): Promise<AuthResponse> {
    ...
    const result = await Database.query(
      `SELECT id, email, password_hash, temp_password_hash, verification_expires_at, created_at
       FROM users 
       WHERE email = $1 
    ...
    if (expiresAt < new Date()) {
      // Clear expired code
      await Database.query(
        'UPDATE users SET verification_code = NULL, verification_expires_at = NULL WHERE id = $1',
        [user.id]
      );
   ...
   */
  static async resendRegistrationOTP(email: string): Promise<void> {
