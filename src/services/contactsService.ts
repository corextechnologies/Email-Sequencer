...
export class ContactsService {
  ...
  async createContact(userId: number, data: CreateContactRequest): Promise<Contact> {
    const query = `
  ...
  async getContacts(userId: number, params: ContactsListRequest): Promise<ContactsListResponse> {
    const {
      page = 1,
      limit = 20,
      search,
  ...
  // Update contact
  async updateContact(userId: number, contactId: number, data: UpdateContactRequest): Promise<Contact | null> {
    const fields: string[] = [];
    const values: any[] = [];
    let paramIndex = 1;
  ...
  async checkDuplicate(userId: number, email: string): Promise<Contact | null> {
  ...
  async importContacts(userId: number, importData: ImportRequest): Promise<ImportResponse> {
  ...
  async importFromCSV(userId: number, csvData: Buffer): Promise<{
    success: number;
    failed: number;
    duplicates: number;
  ...
  async previewCSV(csvData: Buffer): Promise<{
    preview: any[];
    totalRows: number;
    columns: string[];
  }> {
  ...
  private validateCSVRow(row: any): CreateContactRequest {
  ...
  private findFieldValue(row: any, fieldNames: string[]): string | undefined {
  ...
  async previewVCF(vcfData: Buffer): Promise<{
    contacts: any[];
  ...
  async importFromVCF(userId: number, vcfData: Buffer): Promise<{
    success: number;
    failed: number;
    duplicates: number;
    errors: Array<{ row: number; error: string; data: any }>;
  ...
  private convertVCFToContact(vcfContact: any): any {
  ...
  async importPhoneContacts(userId: number, contacts: any[]): Promise<{
    success: number;
    failed: number;
    duplicates: number;
    errors: Array<{ row: number; error: string; data: any }>;
