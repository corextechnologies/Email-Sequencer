 ...
 */
function buildEnrichmentPrompt(contact: any): string {
 ...
 */
function cleanAndParseEnrichment(aiText: string): EnrichmentResult | null {
  try {
...
export async function enrichContactWithAI(
  ...
  try {
    ...
    const contactRes = await Database.query(
      `SELECT id, first_name, last_name, email, company, job_title, phone, 
              social_link, notes 
       FROM contacts 
    ...
    const existingEnrichment = await Database.query(
      'SELECT id FROM enriched_data WHERE contact_id = $1',
      [contactId]
    );
