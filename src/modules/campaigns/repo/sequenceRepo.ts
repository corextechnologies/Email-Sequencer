...
export class SequenceRepo {
	...
	async listSteps(userId: number, campaignId: number): Promise<SequenceStep[]> {
		const result = await this.db.query(
			`SELECT s.id, s.campaign_id, s.step_index, s.delay_hours, s.from_email_account_id,
	...
	async createSteps(userId: number, campaignId: number, steps: Omit<SequenceStep,'id'|'created_at'|'updated_at'|'campaign_id'>[]): Promise<SequenceStep[]> {
		return await this.withTransaction(async (client) => {
			...
			if (guard.rowCount === 0) { throw new Error('CAMPAIGN_NOT_FOUND'); }
			if (guard.rows[0].status === 'running') { throw new Error('CAMPAIGN_RUNNING'); }
			...
			for (const s of steps) {
				const result = await client.query(
					`INSERT INTO sequence_steps (campaign_id, step_index, delay_hours, from_email_account_id, subject_template, body_template, prompt_key, enabled)
					...
					[campaignId, nextIndex++, s.delay_hours, s.from_email_account_id ?? null, s.subject_template, s.body_template, s.prompt_key ?? null, s.enabled]
				);
	...
	async updateStep(userId: number, campaignId: number, stepId: number, update: Partial<SequenceStep>): Promise<SequenceStep | null> {
		return await this.withTransaction(async (client) => {
			const guard = await client.query(`SELECT status FROM campaigns WHERE id = $1 AND user_id = $2 FOR UPDATE`, [campaignId, userId]);
			if (guard.rowCount === 0) { throw new Error('CAMPAIGN_NOT_FOUND'); }
			if (guard.rows[0].status === 'running') { throw new Error('CAMPAIGN_RUNNING'); }
			...
			const values: any[] = [];
			let idx = 1;
			if (update.delay_hours !== undefined) { fields.push(`delay_hours = $${idx++}`); values.push(update.delay_hours); }
			if (update.from_email_account_id !== undefined) { fields.push(`from_email_account_id = $${idx++}`); values.push(update.from_email_account_id); }
			if (update.subject_template !== undefined) { fields.push(`subject_template = $${idx++}`); values.push(update.subject_template); }
			if (update.body_template !== undefined) { fields.push(`body_template = $${idx++}`); values.push(update.body_template); }
			if (update.prompt_key !== undefined) { fields.push(`prompt_key = $${idx++}`); values.push(update.prompt_key); }
			if (update.enabled !== undefined) { fields.push(`enabled = $${idx++}`); values.push(update.enabled); }
			if (fields.length === 0) {
			...
			const result = await client.query(
				`UPDATE sequence_steps SET ${fields.join(', ')} WHERE id = $${idx++} AND campaign_id = $${idx++}
				 RETURNING id, campaign_id, step_index, delay_hours, from_email_account_id, subject_template, body_template, prompt_key, enabled,
				 to_char(created_at, 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') as created_at,
				 to_char(updated_at, 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') as updated_at`,
				values
	...
	async deleteStep(userId: number, campaignId: number, stepId: number): Promise<boolean> {
		return await this.withTransaction(async (client) => {
			const guard = await client.query(`SELECT status FROM campaigns WHERE id = $1 AND user_id = $2 FOR UPDATE`, [campaignId, userId]);
			if (guard.rowCount === 0) { throw new Error('CAMPAIGN_NOT_FOUND'); }
			if (guard.rows[0].status === 'running') { throw new Error('CAMPAIGN_RUNNING'); }
	...
	async reorder(userId: number, campaignId: number, stepIds: number[]): Promise<SequenceStep[]> {
		return await this.withTransaction(async (client) => {
			const guard = await client.query(`SELECT status FROM campaigns WHERE id = $1 AND user_id = $2 FOR UPDATE`, [campaignId, userId]);
			if (guard.rowCount === 0) { throw new Error('CAMPAIGN_NOT_FOUND'); }
			if (guard.rows[0].status === 'running') { throw new Error('CAMPAIGN_RUNNING'); }
			...
			const allowed = new Set<number>(dbIds.rows.map((r: any) => r.id));
