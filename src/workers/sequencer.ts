...
import { JobQueue } from '../modules/campaigns/services/queue';
import { MailerService } from '../modules/campaigns/services/mailer';
import { renderTemplate, buildUnsubscribeHeaders, trackingPixelHtml, smartTrackingPixel } from '../modules/campaigns/services/rendering';
import { formatEmailForSending } from '../utils/emailFormatter';
import { startReplyDetector } from './replyDetector';

async function checkCampaignCompletion(db: any, campaignId: number): Promise<void> {
  try {
    ...
    if (parseInt(total_contacts) > 0 && parseInt(processed_contacts) === parseInt(total_contacts)) {
      const updateResult = await db.query(
        `UPDATE campaigns SET status = 'completed' WHERE id = $1 AND status = 'running' RETURNING id, status`,
        [campaignId]
      );
...
async function run() {
  ...
  startReplyDetector().catch(console.error);
  ...
  const queue = new JobQueue(Database.getPool());
  ...
  while (running) {
    try {
      ...
      const delay = Math.min(
        BASE_DELAY_MS * Math.pow(2, Math.min(consecutiveErrors - 1, 4)),
        MAX_DELAY_MS
      );
...
async function handleSendEmail(queue: JobQueue, job: any) {
  ...
  try {
    ...
    const sequenceCheck = await db.query(
      `SELECT ce.*, cc.current_email_number, cc.total_emails, cc.sequence_started_at
    ...
    if (hasSequence) {
      ...
      if (!emailToSend) {
        ...
        await db.query(
          `UPDATE campaign_contacts SET status='completed' WHERE campaign_id=$1 AND contact_id=$2`,
          [campaign_id, contact_id]
        );
        await queue.complete(job.id);
        return;
    ...
    const accountRes = await db.query(
      `SELECT id, provider, smtp_host, smtp_port, username, encrypted_password, is_active, user_id
       FROM email_accounts WHERE id = $1 AND user_id = $2`,
      [campaign.from_email_account_id, userId]
    );
    ...
    if (emailToSend) {
      ...
      subject = renderTemplate(emailToSend.subject, ctx);
      rawBody = renderTemplate(emailToSend.body, ctx);
    } else {
      ...
      subject = renderTemplate(campaign.email_subject, ctx);
      rawBody = renderTemplate(campaign.email_body, ctx);
    }
    ...
    html = formatEmailForSending(rawBody, {
    ...
    if (process.env.FEATURE_TRACKING_PIXEL === 'true') {
      ...
      html += smartTrackingPixel(pixelUrl, {
        method: pixelMethod as 'invisible' | 'visible' | 'auto'
      });
    ...
    const headers = buildUnsubscribeHeaders({ mailto: `mailto:unsubscribe@example.com?subject=${encodeURIComponent(token)}`, http: unsubscribeLink });
        ...
        if (nextEmailNum <= totalEmails) {
          ...
          if (nextEmail) {
            ...
            await db.query(
              ...
              [nextEmailNum, nextSendDate, campaign_id, contact_id]
            );
          ...
          await db.query(
            ...
            [nextEmailNum, campaign_id, contact_id]
          );
        }
      } else {
        // Single email (no sequence)
        await db.query(
          `UPDATE campaign_contacts SET status='completed', last_email_sent_at = NOW() WHERE campaign_id=$1 AND contact_id=$2`,
          [campaign_id, contact_id]
        );
      }
