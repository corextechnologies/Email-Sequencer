...
class CampaignRepo {
    ...
    async createCampaign(userId, input) {
        const { name, email_subject, email_body, from_email_account_id } = input;
        const result = await this.db.query(`INSERT INTO campaigns (user_id, name, email_subject, email_body, from_email_account_id)
			 VALUES ($1, $2, $3, $4, $5)
			 RETURNING id, user_id, name, status, email_subject, email_body, from_email_account_id, to_char(created_at, 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') as created_at, to_char(updated_at, 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') as updated_at`, [userId, name, email_subject, email_body, from_email_account_id]);
        return result.rows[0];
    }
    async listCampaigns(userId) {
        const result = await this.db.query(`SELECT id, user_id, name, status, email_subject, email_body, from_email_account_id,
    ...
    async getById(userId, campaignId) {
        const result = await this.db.query(`SELECT id, user_id, name, status, email_subject, email_body, from_email_account_id,
			 to_char(created_at, 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') as created_at,
			 to_char(updated_at, 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') as updated_at
			FROM campaigns WHERE id = $1 AND user_id = $2`, [campaignId, userId]);
        return result.rows[0] || null;
    }
    async updateCampaign(userId, campaignId, update) {
        const fields = [];
        const values = [];
        let idx = 1;
        ...
        if (update.email_subject !== undefined) {
            fields.push(`email_subject = $${idx++}`);
            values.push(update.email_subject);
        }
        if (update.email_body !== undefined) {
            fields.push(`email_body = $${idx++}`);
            values.push(update.email_body);
        }
        ...
        if (fields.length === 0) {
            return await this.getById(userId, campaignId);
        }
        values.push(campaignId, userId);
        const result = await this.db.query(`UPDATE campaigns SET ${fields.join(', ')} WHERE id = $${idx++} AND user_id = $${idx++}
    ...
    async updateStatus(userId, campaignId, status) {
        const result = await this.db.query(`UPDATE campaigns SET status = $1 WHERE id = $2 AND user_id = $3 RETURNING id, user_id, name, status,
			 to_char(created_at, 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') as created_at,
			 to_char(updated_at, 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') as updated_at`, [status, campaignId, userId]);
        return result.rows[0] || null;
    }
    async checkAndCompleteCampaign(campaignId) {
        ...
        if (parseInt(total_contacts) > 0 && parseInt(processed_contacts) === parseInt(total_contacts)) {
            await this.db.query(`UPDATE campaigns SET status = 'completed' WHERE id = $1 AND status = 'running'`, [campaignId]);
    ...
    async getCampaignMetrics(userId, campaignId) {
        ...
        const campaignCheck = await this.db.query(`SELECT id FROM campaigns WHERE id = $1 AND user_id = $2`, [campaignId, userId]);
     ...
     */
    async getEmailOpenMetrics(campaignId) {
        // First verify the campaign exists
        const campaignCheck = await this.db.query(`SELECT id FROM campaigns WHERE id = $1`, [campaignId]);
        if (campaignCheck.rows.length === 0) {
            return null;
        }
