...
exports.generateEmailSequenceForContact = generateEmailSequenceForContact;
const connection_1 = require("../database/connection");
const universalLlmService_1 = require("./universalLlmService");
function buildEmailSequencePrompt(contact, persona, sequenceParams, enrichmentData) {
...
function cleanAndParseEmailSequence(aiText) {
    if (!aiText)
        return null;
...
async function generateEmailSequenceForContact(userId, contactId, provider, apiKey, sequenceParams) {
    // 1) Fetch contact
    ...
    try {
        const enrichmentRes = await connection_1.Database.query(`SELECT professional_context, recent_activity, company_insights, 
    ...
    const personasRes = await connection_1.Database.query('SELECT id, name, description, industry, role, company_size, location, current_challenges, interests_priorities, communication_style FROM personas WHERE user_id = $1 ORDER BY created_at DESC', [userId]);
    ...
    let aiResponse;
    try {
        aiResponse = await universalLlmService_1.UniversalLlmService.generateText({
            provider,
            apiKey,
            prompt,
            maxTokens: 2000,
...
//# sourceMappingURL=emailSequenceGeneratorService.js.map