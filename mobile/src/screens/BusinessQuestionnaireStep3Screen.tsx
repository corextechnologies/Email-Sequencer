...
const BusinessQuestionnaireStep3Screen: React.FC<Props> = ({ navigation }) => {
    const { data, updateData } = useQuestionnaire();
    const [targetMarket, setTargetMarket] = useState(data.targetAudience);

    const isFormValid = useMemo(() => targetMarket.trim().length > 0, [targetMarket]);
    useEffect(() => {
        if (data.targetAudience) {
            setTargetMarket(data.targetAudience);
        }
    }, [data.targetAudience]);
    ...
    const totalSteps = 6;
    const currentStep = 3; // change this number for each screen
    ...
    return (
        <SafeAreaView style={styles.safeArea}>
            ...
            <ScrollView style={styles.container} contentContainerStyle={styles.content}>
                {/* Tell us about your business + AI Scan */}
                ...
                {/* Target Market + Primary Audience Inputs */}
                <View style={styles.section}>
                    <Text style={styles.sectionTitle}>Target Marketing</Text>

                    <Text style={styles.label}>
                        Describe Target Market <Text style={styles.required}>*</Text>
                    </Text>
                    <TextInput
                        value={targetMarket}
...
export default BusinessQuestionnaireStep3Screen;
